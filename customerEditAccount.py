...
class CustomerEditAcc:
    def __init__(self, cust_app):
        ...
        def update_cust():
            if name_text.get() == '' or lastname_text.get() == '' or email_text.get() == '' or \
                    access_key_text.get() == '' or phone_text.get() == '':
        ...
        def delete_acc():
            important = tkinter.messagebox.askyesno("Registration Panel",
                                                    "Do you want to delete your account? You can't undo this")
            if important > 0:
        ...
        name_entry = tk.Entry(data_frame, textvariable=name_text, font=FONT_SIZE, bg=shared.LISTBOX_COLOR)
        ...
        lastname_entry = tk.Entry(data_frame, textvariable=lastname_text, font=FONT_SIZE, bg=shared.LISTBOX_COLOR)
        ...
        access_key_label = tk.Label(data_frame, text='Access Key', font=FONT_SIZE, bg=shared.BG_COLOR)
        ...
        phone_label = tk.Label(data_frame, text='Phone Number', font=FONT_SIZE, pady=20, bg=shared.BG_COLOR)
        phone_label.grid(row=2, column=0, sticky=tk.E)
        phone_entry = tk.Entry(data_frame, textvariable=phone_text, font=FONT_SIZE, bg=shared.LISTBOX_COLOR)
        ...
        add_btn = tk.Button(button_frame, text='Update Account', width=12, command=update_cust, bg=shared.BG_COLOR)
        add_btn.grid(column=1, row=0, sticky=tk.W)

        delete_btn = tk.Button(button_frame, text='Delete Account', width=12, command=delete_acc, bg=shared.BG_COLOR)
...
if __name__ == "__main__":
    cust_app2 = tk.Tk()
    application = CustomerEditAcc(cust_app2)
